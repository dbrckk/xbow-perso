(()=>{
  ...
  function historicalProgramSignal(handle){
    return hackerOneIntelligence?.program_signals?.[String(handle||'')]||null;
  }
  ...
  function batchCatalogPrograms(){
    ...
    const sort=String(el('h1BatchSort')?.value||'name');
    ...
    return programs.sort((left,right)=>{
      if(sort==='historical_value'){
        const l=Number(historicalProgramSignal(left?.handle)?.historical_value_score||0);
        const r=Number(historicalProgramSignal(right?.handle)?.historical_value_score||0);
        if(r!==l)return r-l;
      }
      return String(left?.name||left?.handle||'').localeCompare(
        String(right?.name||right?.handle||''),
        undefined,
        {sensitivity:'base'}
      );
    });
  ...
  function renderBatchCatalog(){
    ...
    if(!programs.length){
      ...
      for(const program of programs){
        ...
        const meta=document.createElement('div');
        meta.className='muted compact';
        const signal=historicalProgramSignal(program.handle);
        const parts=[
  ...
  const H1_INTEL_CATEGORY_LABELS={
    access_control:'Accès / IDOR',
    auth_session:'Auth / session',
    api_graphql:'API / GraphQL',
    business_logic:'Logique métier / race',
    xss_client:'XSS / client',
    ssrf_oob:'SSRF / OOB',
    injection_rce:'Injection / RCE',
    cache_proxy:'Cache / proxy',
    cloud_surface:'Cloud / surface',
    file_path:'Fichiers / chemins',
    information_disclosure:'Divulgation',
    ai_llm:'IA / LLM',
    other:'Autres'
  };

  function formatHistoricalUsd(value){
    const amount=Number(value||0);
    if(!Number.isFinite(amount)||amount<=0)return '—';
    return '    const count=batchSelectedHandles.size;
  ...
  async function initRemoteControlCenter(){
    ...
    try{
      ...
      renderProgramOptions();
      renderBatchCatalog();
      void refreshHackerOneIntelligence(false);
  ...
  el('h1IntelRefresh').addEventListener('click',()=>void refreshHackerOneIntelligence(true));
...
+Math.round(amount).toLocaleString();
  ...
  function renderHackerOneIntelligence(payload){
    hackerOneIntelligence=payload&&typeof payload==='object'?payload:{};
    const categories=Array.isArray(hackerOneIntelligence.categories)
      ?hackerOneIntelligence.categories:[];
    const gaps=Array.isArray(hackerOneIntelligence.capability_gaps)
      ?hackerOneIntelligence.capability_gaps:[];

    const reports=Number(hackerOneIntelligence.report_count||0);
    el('h1IntelReports').textContent=String(reports);
    el('h1IntelTopCategory').textContent=categories.length
      ?(H1_INTEL_CATEGORY_LABELS[categories[0].category]||String(categories[0].category||'—'))
      :'—';

    let maxUsd=0;
    for(const item of Array.isArray(hackerOneIntelligence.high_value_reports)
      ?hackerOneIntelligence.high_value_reports:[]){
      if(String(item?.currency||'')==='USD'){
        maxUsd=Math.max(maxUsd,Number(item?.award_amount||0));
      }
    }
    el('h1IntelMaxAward').textContent=formatHistoricalUsd(maxUsd);

    const categoryList=el('h1IntelCategories');
    categoryList.replaceChildren();
    for(const item of categories.slice(0,8)){
      const row=document.createElement('div');
      row.className='scope-asset-row h1-intel-row';
      const title=document.createElement('strong');
      title.textContent=H1_INTEL_CATEGORY_LABELS[item.category]||String(item.category||'Pattern');
      const meta=document.createElement('span');
      const pieces=[
        String(item.report_count||0)+' report(s)',
        String(item.high_critical_count||0)+' high/critical'
      ];
      if(Number(item.usd_awarded_max||0)>0){
        pieces.push('max public '+formatHistoricalUsd(item.usd_awarded_max));
      }
      meta.textContent=pieces.join(' · ');
      row.append(title,meta);
      categoryList.appendChild(row);
    }
    if(!categoryList.children.length)categoryList.textContent='Aucun pattern disponible.';

    const gapList=el('h1IntelGaps');
    gapList.replaceChildren();
    const actionable=gaps.filter(item=>String(item?.status||'')!=='implemented');
    for(const item of actionable.slice(0,8)){
      const row=document.createElement('div');
      row.className='scope-asset-row h1-intel-row';
      const title=document.createElement('strong');
      title.textContent=String(item.capability||item.category||'capacité');
      const meta=document.createElement('span');
      const tools=Array.isArray(item.recommended_tooling)
        ?item.recommended_tooling.slice(0,2).join(' + '):'';
      meta.textContent=
        (H1_INTEL_CATEGORY_LABELS[item.category]||String(item.category||''))+
        ' · '+String(item.status||'gap')+
        (tools?' · '+tools:'');
      row.append(title,meta);
      gapList.appendChild(row);
    }
    if(!gapList.children.length)gapList.textContent='Aucun gap prioritaire détecté.';

    el('h1IntelCategoryCount').textContent=String(categories.length);
    el('h1IntelGapCount').textContent=String(actionable.length);
    const state=el('h1IntelState');
    state.textContent=reports?reports+' appris':'vide';
    state.className='pill '+(reports?'ok':'warn');
    el('h1IntelUpdated').textContent=
      (hackerOneIntelligence.checked_at
        ?'Dernière analyse '+new Date(hackerOneIntelligence.checked_at).toLocaleString()+'. '
        :'')+
      'Source: Hacktivity publique divulguée; signal historique uniquement.';
    renderBatchCatalog();
  }

  async function refreshHackerOneIntelligence(force=false){
    const button=el('h1IntelRefresh');
    if(button)button.disabled=true;
    try{
      const result=await api('/hackerone/intelligence'+(force?'?refresh=true':''));
      renderHackerOneIntelligence(result);
    }catch(error){
      const state=el('h1IntelState');
      if(state){
        state.textContent='indisponible';
      ...
      if(el('h1IntelUpdated')){
        el('h1IntelUpdated').textContent='Apprentissage indisponible : '+error.message;
      }
    }finally{
      if(button)button.disabled=false;
    }
