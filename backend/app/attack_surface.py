...
def build_attack_surface(graph: ObservationGraph) -> dict[str, Any]:
    ...
    hosts = Counter(item["host"] for item in endpoints if item["host"])
    schemes = Counter(item["scheme"] for item in endpoints if item["scheme"])
    parameter_names = sorted({name for item in endpoints for name in item["parameter_names"]})

    return {
        "assets": sorted(assets, key=lambda item: (item["host"], item["id"])),
        "endpoints": sorted(endpoints, key=lambda item: (item["url"], item["id"])),
