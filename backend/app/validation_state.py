...
class ValidationState:
    ...
    def unevidenced_finding_ids(self) -> frozenset[str]:
        return self.observed_independent_finding_ids - self.evidence_backed_independent_finding_ids
    ...
    def all_evidence_backed_independently(self) -> bool:
        return bool(self.finding_ids) and self.finding_ids == self.evidence_backed_independent_finding_ids


def analyze_validation_state(graph: Any) -> ValidationState:
    """Compute finding validation state once from an observation graph.

    A finding is considered evidence-backed only when an independent observed
    validation has at least one evidence observation directly attached to that
    validation node. This keeps validation state aligned with report/evidence
    readiness and avoids treating an unsubstantiated validator assertion as
    fully corroborated.
    """
    ...
    return ValidationState(
        ...
        evidence_backed_independent_finding_ids=frozenset(evidence_backed_independent),
    )
...
def evidence_backed_independent_finding_ids(graph: Any) -> set[str]:
    """Return findings whose independent observed validation has attached evidence."""
...
def has_evidence_backed_independent_validation(graph: Any, finding_id: str) -> bool:
    return finding_id in analyze_validation_state(graph).evidence_backed_independent_finding_ids
