...
It does not grant PentAGI unrestricted terminal, browser, raw-network, shell, or file execution access for XBOW-managed bug-bounty flows.
...
Add a scanner-specific Docker image, `backend/Dockerfile.scanner`, and point the `scanner-worker` Compose service at it. This image is the only image required to contain Nuclei.
...
The scanner image contains a read-only preloaded template tree or an equivalent immutable artifact. The worker passes the explicit template directory to Nuclei and keeps update checks disabled.
...
The worker continues constructing the command itself. Campaign or PentAGI input cannot inject raw Nuclei flags.
...
- explicit pinned template directory supplied by XBOW.

If the pinned template set introduces tags or behavior incompatible with the allowed policy, scanner admission blocks rather than broadening execution.

## 5. PentAGI integration architecture

### 5.1 Role separation and reviewed upstream contract

PentAGI becomes a planner/orchestrator for authorized campaigns, not a direct executor of arbitrary target actions.

The initial reviewed upstream source contract is PentAGI revision `ea665308baaff015b226f308438a68d929d0f29b`. A later PentAGI upgrade is a security-relevant change and requires a new contract review before XBOW active capability can accept it.

For XBOW-managed flows, the PentAGI `Functions` object must disable these built-ins for all agent contexts by omitting `context` from each disable rule:

- `terminal`;
- `browser`;
- `file`.

Those names are part of the reviewed PentAGI function configuration contract. XBOW must not rely only on prompt text to suppress them.

Because PentAGI can evolve and add new built-in capabilities, deployment must pin the PentAGI instance to the reviewed revision/image digest. XBOW configuration records the reviewed revision/digest identity, and production preflight treats a PentAGI upgrade as blocked until the contract is reviewed and the expected identity is deliberately updated.

An externally managed PentAGI deployment that cannot provide immutable deployment/version evidence is not production-active-capable under this design; it may remain preview/status-only.

PentAGI receives only a constrained set of XBOW-backed external functions.

### 5.2 Exact REST transport contract

The current GraphQL `createFlow` execution plan is replaced for active flows by PentAGI's authenticated REST flow API.

Against the reviewed PentAGI contract, XBOW uses these exact routes under the configured HTTPS base URL:

- `POST /api/v1/flows/` to create a flow;
- `GET /api/v1/flows/{flowID}` to reconcile current flow state;
- `PUT /api/v1/flows/{flowID}` with `action: "stop"` to request remote cancellation.

The `POST /api/v1/flows/` JSON body is server-constructed and contains:

- `input`: the bounded campaign/orchestration prompt;
- `provider`: the configured provider name;
- `functions`: the reviewed PentAGI Functions object;
- `resource_ids`: omitted or an empty allowlisted list in the initial release.

The Functions object contains exactly:

- `token`: dedicated XBOW external-function bearer token;
- `disabled`: the reviewed direct-tool deny list;
- `functions`: the exact XBOW external-function definitions.

PentAGI's reviewed contract sends the configured Functions token as `Authorization: Bearer <token>` when invoking external functions. XBOW uses a separate credential for authenticating XBOW -> PentAGI REST requests.

Keep HTTPS-only endpoint validation, no redirects, bounded response size, bounded total deadline, no credentials in URL, exact expected API paths, and server-side token loading.
...
The function definitions and JSON schemas are generated by XBOW code; campaign input cannot add, rename, or replace functions.

No generic `run_command`, `fetch_url`, `browser_action`, `raw_http`, `shell`, arbitrary file operation, or arbitrary scanner-flag function is exposed.
...
- reviewed PentAGI contract/revision identity;
- model provider;
- creation timestamp;
- status;
...
The value PentAGI uses when calling an XBOW function is an opaque binding identifier, not a user-guessable campaign ID that can be substituted to cross campaign boundaries.
...
2. resolve the opaque flow binding;
...
7. reject any requested rate above the campaign policy or the applicable XBOW local ceiling;
8. reject malformed/unknown parameters rather than forwarding them;
9. build the worker payload internally;
10. enqueue with a deterministic dedupe key;
11. return a bounded status object.

A PentAGI request must not contain an arbitrary executable command or arbitrary destination URL.
...
For Nuclei, the requested rate must be less than or equal to both the campaign policy rate and the local autonomy cap. A request above either limit is rejected explicitly; XBOW does not silently clamp an invalid PentAGI request.

The admitted exact rate is then translated by the existing Nuclei plan builder into conservative Nuclei rate-limit arguments.
...
- XBOW sends `PUT /api/v1/flows/{flowID}` with `action: "stop"` to the bound PentAGI flow;
- the stop operation is locally idempotent/reconcilable;
- failure to confirm remote stop is surfaced as an operational alert and does not re-enable local execution.

The status poller keeps reconciling remote state until a terminal remote state or configured reconciliation deadline is reached.

### 5.8 Remote create idempotency and ambiguous failure

Preserve the current rule that mutating remote flow creation is not automatically retried after an ambiguous transport failure unless PentAGI provides a server-side idempotency mechanism that XBOW can verify.

XBOW keeps a deterministic local idempotency key and stores enough information to reconcile whether a flow was created.

If the HTTP result is ambiguous, the job enters a distinct reconciliation/manual-review state rather than running `POST /api/v1/flows/` again.
...
The XBOW external-function endpoint used by PentAGI has a separate integration credential from the human-facing `XBOW_API_TOKEN` and from the credential XBOW uses to call PentAGI.

The external-function credential must be:
...
Function requests additionally require an opaque flow binding, so possession of the integration token alone does not select an arbitrary campaign.

If future PentAGI releases support request signing, XBOW may add HMAC request signatures and replay protection. This is not required for the first active implementation if the integration is HTTPS-only, bearer-token authenticated, and bound per flow, but the interface must not prevent adding signatures later.
...
`GET /api/capabilities` stops reporting PentAGI as `preview_only` only when every required active contract is satisfied.
...
- worker enabled;
- reviewed REST transport enabled;
- dedicated external-function channel configured;
- direct `terminal`, `browser`, and `file` tools disabled for all XBOW flow contexts;
- persistent binding storage available;
- deployment identity matches the reviewed PentAGI revision/image contract;
- rate and scope enforcement remain XBOW-side.

Capability output exposes non-secret block reasons for every missing gate.

Nuclei capability reporting similarly exposes version/template/runtime block reasons without leaking host secrets.
...
- unreviewed PentAGI revision/deployment identity: active mode blocked;
...
- reviewed PentAGI contract identity accepted/rejected;
...
Metrics include counts and age for unresolved PentAGI reconciliation items and scanner runtime failures.
...
### 11.1 Compose and configuration

`scanner-worker` uses the dedicated scanner Dockerfile/image.

PentAGI creator and status workers remain behind explicit Compose profiles.
...
- PentAGI external-function integration token or vault key name;
- XBOW external-function base URL reachable from PentAGI;
- reviewed PentAGI revision or immutable image identity;
- explicit PentAGI REST-contract enablement after review.

All defaults remain fail closed.

### 11.2 PentAGI deployment pin

The reviewed initial PentAGI contract is tied to upstream revision `ea665308baaff015b226f308438a68d929d0f29b`.

For a production-active deployment, the PentAGI image or deployment artifact must be immutable and traceable to the reviewed revision. Mutable tags such as `latest` are not acceptable evidence.

If PentAGI is managed outside XBOW and its immutable identity cannot be established, XBOW keeps PentAGI active execution disabled while allowing non-mutating preview/status capabilities that are safe under the existing model.
...
- flow creation uses `POST /api/v1/flows/` rather than the old GraphQL mutation;
- flow creation payload disables `terminal`, `browser`, and `file` globally;
- flow creation contains only reviewed XBOW external functions;
- PentAGI's external-function bearer token is never returned in API responses/logs;
...
- over-policy rate requests are rejected rather than clamped;
...
- stop propagation uses the reviewed REST patch action and is idempotent locally;
- unreviewed PentAGI deployment identity blocks active capability;
...
2. create PentAGI flow through the reviewed REST contract;
3. persist binding;
4. PentAGI invokes a bounded `request_nuclei_scan` external function with its bearer token and opaque binding;
5. XBOW revalidates current policy and creates the exact internal scan job;
6. simulated scanner result is ingested;
7. PentAGI reads bounded findings/status;
8. campaign cancellation causes subsequent action denial and `action: "stop"` remote request.
...
- new focused module(s) for PentAGI flow bindings and external-function authorization;
- storage abstraction/implementations for flow bindings;
- focused PentAGI integration API router rather than expanding `main.py` when practical;
...
Implementation is split into reviewable, independently safe increments on the feature branch or a small PR series:
...
3. reviewed PentAGI revision/deployment identity gate;
4. persistent flow binding + dedicated integration authentication;
5. XBOW external-function endpoint with read-only functions first;
6. bounded mutating functions backed by existing XBOW queues;
7. active PentAGI admission gate switched from preview-only only after the preceding contracts are verified;
8. cancellation/stop/reconciliation handling;
9. end-to-end fake-PentAGI integration test and docs.
...
- PentAGI can create a real flow using `POST /api/v1/flows/` and the reviewed `Functions` contract;
- active mode is tied to an immutable reviewed PentAGI deployment identity;
- the XBOW-managed flow disables PentAGI `terminal`, `browser`, and `file` built-ins globally;
...
- over-policy PentAGI rate requests are rejected explicitly;
...
Pinning the reviewed PentAGI contract also prevents a future upstream tool addition from silently defeating the direct-tool deny list.
